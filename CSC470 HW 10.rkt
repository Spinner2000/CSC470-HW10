...
#reader(lib "htdp-advanced-reader.ss" "lang")((modname |CSC470 HW 10|) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #t #t none #f () #f)))
;***** Environment Stuff *****
(define resolve
  (lambda (varName env)
    (cond
...
(define run-parsed-code
  (lambda (parsed-no-code env)
    (cond
      ...
      ((eq? (car parsed-no-code) 'local-vars)
       (resolve (cadr parsed-no-code) env))
...
(run-parsed-code '(local-vars ((a 2) (b 3)) (local-vars ((c 4) (d 5) (e b)) (do-mathy-stuff + b a))) env)